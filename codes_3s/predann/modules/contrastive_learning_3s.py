...
class EEGContrastiveLearning(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        label = batch[2]
        ce_loss_r, acc_r = self._shared_step(label, y_eeg)
        ce_loss_c, acc_c = self._shared_step(label, y_audio)
        ...
        if self.current_epoch == self.trainer.max_epochs - 1:
            self.last_epoch_train_embeddings.append(z_eeg.cpu().detach().numpy())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("Accuracy/valid_eeg", acc_r)
        self.log("Accuracy/valid_audio", acc_c)
