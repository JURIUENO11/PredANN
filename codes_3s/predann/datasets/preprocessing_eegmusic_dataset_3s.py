...
class Preprocessing_EEGMusic_dataset(Dataset):
    ...
    def getitem(self, n, isClip=True):
        ...
        if isClip:
            eeg_length = min(eeg.size(1), audio.size(1))
            ...
            audio_start = int(eeg_start/125 * 44100)-int((self.audio_clip-3)/2*44100)
        ...
        if self.transform != None:
            ...
            audio = self.transform(audio, sample_rate=self.eeg_sample_rate)
    ...
    def __getitem__(self, n: int) -> Tuple[Tensor, Tensor]:
        eeg, audio, song_id= self.getitem(n)

        return eeg, audio, song_id
