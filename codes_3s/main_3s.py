...
if __name__ == "__main__":
    ...
    train_log = pd.DataFrame(columns=["Loss/train", "Accuracy/train_raw", "Accuracy/train_clean"])
    valid_log = pd.DataFrame(columns=["Loss/valid", "Accuracy/valid_raw", "Accuracy/valid_clean"])
    ...
    train_dataset.set_other_parameters(args.eeg_length, args.audio_clip_length,args.split_seed, args.class_song_id,args.shifting_time, args.start_position)
    random.seed(42)
    ...
    valid_dataset.set_other_parameters(args.eeg_length,args.audio_clip_length, args.split_seed, args.class_song_id,args.shifting_time, args.start_position)
    random.seed(42)
    ...
    if args.dataset == "preprocessing_eegmusic":
        encoder_raw_e = SampleCNN2DEEG(
            out_dim=train_dataset.labels(),
            kernal_size=3,
        ) 
        encoder_clean_e = SampleCNN2DEEG(
            out_dim=train_dataset.labels(),
            kernal_size=3,
        )
    ...
    module = EEGContrastiveLearning(valid_dataset, args, encoder_raw_e, encoder_clean_e)
