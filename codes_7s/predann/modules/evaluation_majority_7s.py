...
class EEGContrastiveLearning(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for raw_eeg, clean_eeg in zip(raw_data, clean_data):
            y_r, y_c, z_r, z_c = self.forward(raw_eeg, clean_eeg)
        ...
        self.log("Accuracy/valid_raw", acc_r_av)
        self.batch_accuracies.append(acc_r_av.item())
        self.log("Accuracy/valid_clean", acc_c_av)
        self.batch_accuracies_c.append(acc_c_av.item())

        new_valid_log = pd.DataFrame({"Loss/valid": [loss.item()], "Accuracy/valid_raw": [acc_r_av.item()], "Accuracy/valid_clean": [acc_c_av.item()]})
