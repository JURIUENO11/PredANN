...
class EEGContrastiveLearning(LightningModule):
    ...
    def forward(self, r, c):
        y_r, z_r = self.encoder_eeg(r)
        y_c, z_c = self.encoder_audio(c)
        return y_r, y_c, z_r, z_c
    

    def training_step(self, batch, batch_idx):
        eeg, audio = batch[:2]
        y_r, y_c, z_r, z_c = self.forward(eeg, audio)
        ...
        self.log("Accuracy/train_eeg", acc_r)
        self.log("Accuracy/train_audio", acc_c)

        new_train_log = pd.DataFrame({"Loss/train": [loss.item()], "Accuracy/train_eeg": [acc_r.item()], "Accuracy/train_audio": [acc_c.item()]})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for eeg, audio in zip(eeg_data, audio_data):
            ...
            if self.hparams.detach_z_c:
                z_c = z_c.detach()
            clip_loss = self.criterion(z_r, z_c)
            y_r=torch.nn.functional.softmax(y_r, dim=1) 
            ce_loss_r, preds_r = self._shared_step(label, y_r)
            ce_loss_c, preds_c = self._shared_step(label, y_c)
            y_r_list.append(preds_r)
            y_c_list.append(preds_c)
        for index in range(len(y_r_list[0])):  
            column = [sublist[index].item() for sublist in y_r_list]
            counter = Counter(column)
            most_common = counter.most_common(1)[0][0]
            results_r.append(most_common)  
        for index in range(len(y_c_list[0])):  
            column = [sublist[index].item() for sublist in y_c_list]
            counter = Counter(column)
            most_common = counter.most_common(1)[0][0]
            results_c.append(most_common)  
        results_r = torch.tensor(results_r, device=label.device)
        results_c = torch.tensor(results_c, device=label.device)
        
        acc_r_av = (results_r == label).sum() / results_r.size(0)
        acc_c_av = (results_c == label).sum() / results_c.size(0)
        ...
        if self.current_epoch == self.trainer.max_epochs - 1:
            self.last_epoch_valid_embeddings.append(z_r.cpu().detach().numpy())
    ...
    def configure_criterion(self):
        ...
        if self.hparams.loss_function == "nt_xent_lmd":
            print('use NT_Xent_LMD as criterion')
            criterion = NT_Xent_LMD(batch_size, self.hparams.temperature, world_size=1)
