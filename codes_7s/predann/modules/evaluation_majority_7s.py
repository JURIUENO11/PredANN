...
class EEGContrastiveLearning(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for eeg, audio in zip(eeg_data, audio_data):
            ...
            y_eeg=torch.nn.functional.softmax(y_eeg, dim=1) 
            ce_loss_r, preds_r = self._shared_step(label, y_eeg)
            ce_loss_c, preds_c = self._shared_step(label, y_audio)
            y_eeg_list.append(preds_r)
            y_audio_list.append(preds_c)
        for index in range(len(y_eeg_list[0])):  
            column = [sublist[index].item() for sublist in y_eeg_list]
            counter = Counter(column)
            most_common = counter.most_common(1)[0][0]
            results_r.append(most_common)  
        for index in range(len(y_audio_list[0])):  
            column = [sublist[index].item() for sublist in y_audio_list]
            counter = Counter(column)
            most_common = counter.most_common(1)[0][0]
