...
class EEGContrastiveLearning(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_c=self.tensor_calculate(y_c_list)
        y_r=self.tensor_calculate(y_r_list)
        z_c=self.tensor_calculate(z_c_list)
        z_r=self.tensor_calculate(z_r_list)
        if self.hparams.detach_z_c:
          z_c = z_c.detach()
        clip_loss = self.criterion(z_r, z_c)
        ce_loss_r, acc_r_av = self._shared_step(label, y_r)
        ce_loss_c, acc_c_av = self._shared_step(label, y_c)
        loss = self.hparams.weight_clip*clip_loss + self.hparams.weight_r*ce_loss_r + self.hparams.weight_c*ce_loss_c
    ...
    def tensor_calculate(self,list):
        ...
        tensor=torch.stack(softmax_list).max(dim=0).values
        return tensor
