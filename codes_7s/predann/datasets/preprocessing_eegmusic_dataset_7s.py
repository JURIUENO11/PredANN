    ...
    def getitem(self, n, isClip=True):
        ...
        if isClip:  
            ...
            for i in range(int((evaluation_length-375)/125+1)):
                raw_data = raw_eeg[:, eeg_start+int(self.eeg_sample_rate*i*1) : eeg_start + self.eeg_clip_length+int(self.eeg_sample_rate*i*1)].clone()         
                clean_data = clean_eeg[:, int(int(audio_start)+44100*i*1) : int(int(audio_start + self.audio_clip * 44100)+44100*i*1)].clone()
                debug_logger.debug(f"Clean_eeg, {clean_data.shape}")
                

                if self.transform != None:
                    raw_data = raw_data.to('cpu').detach().numpy().copy()
                    raw_data = self.transform(raw_data, sample_rate=self.eeg_sample_rate)
                    raw_data = torch.from_numpy(raw_data.astype(np.float32)).clone()

                    clean_data = clean_data.to('cpu').detach().numpy().copy()
                    clean_data = self.transform(clean_data, sample_rate=self.eeg_sample_rate)
                    clean_data = torch.from_numpy(clean_data.astype(np.float32)).clone()
                
                if self.eeg_normalization == "channel_mean":
                    raw_data = self.normalize_EEG(raw_data)
                elif self.eeg_normalization == "all_mean":
                    raw_data = self.normalize_EEG_2(raw_data)
                elif self.eeg_normalization == "constant_multiple":
                    raw_data = self.normalize_EEG_3(raw_data)
                elif self.eeg_normalization == "MetaAI":
                    raw_data = self.normalize_EEG_4(raw_data, self.clamp_value)
                
                if self.eeg_clip_length == 375:
                    padding = (int((3**6-self.eeg_clip_length)/2), int((3**6-self.eeg_clip_length)/2))
                    raw_data = F.pad(raw_data, padding)
                    if raw_data.shape[1] != 3**6:
                        padding = (0, 3**6-raw_data.shape[1])
                        raw_data = F.pad(raw_data, padding)
                    if self.audio_clip <= 4:
                        padding2 = (int((3**11-self.audio_length)/2), int((3**11-self.audio_length)/2))
                        clean_data = F.pad(clean_data, padding2)
                        if clean_data.shape[1] != 3**11:
                            padding2 = (0, 3**11-clean_data.shape[1])
                            clean_data = F.pad(clean_data, padding2)
                    if self.audio_clip >= 4:
                        padding2 = (int((3**12-self.audio_length)/2), int((3**12-self.audio_length)/2))
                        clean_data = F.pad(clean_data, padding2)
                        if clean_data.shape[1] != 3**12:
                            padding2 = (0, 3**12-clean_data.shape[1])
                            clean_data = F.pad(clean_data, padding2)
                if self.eeg_clip_length == 125:
                    padding = (int((3**5-self.eeg_clip_length)/2), int((3**5-self.eeg_clip_length)/2))
                    raw_data = F.pad(raw_data, padding)
                    if raw_data.shape[1] != 3**5:
                        padding = (0, 3**5-raw_data.shape[1])
                        raw_data = F.pad(raw_data, padding)
                    padding2 = (int((3**10-self.audio_length/3)/2), int((3**10-self.audio_length/3)/2))
                    clean_data = F.pad(clean_data, padding2)
                    if clean_data.shape[1] != 3**10:
                        padding2 = (0, 3**10-clean_data.shape[1])
                        clean_data = F.pad(clean_data, padding2)
                if self.eeg_clip_length == 1125:
                    padding = (int((3**7-self.eeg_clip_length)/2), int((3**7-self.eeg_clip_length)/2))
                    raw_data = F.pad(raw_data, padding)
                    if raw_data.shape[1] != 3**7:
                        padding = (0, 3**7-raw_data.shape[1])
                        raw_data = F.pad(raw_data, padding)
                    padding2 = (int((3**12-self.audio_length)/2), int((3**12-self.audio_length)/2))
                    clean_data = F.pad(clean_data, padding2)
                    if clean_data.shape[1] != 3**12:
        ...
        return Raw_list, Clean_list, label, subject


    def __getitem__(self, n: int) -> Tuple[Tensor, Tensor]:
        raw_list, clean_list, song_id, subjects = self.getitem(n)
        return raw_list, clean_list, song_id, subjects
